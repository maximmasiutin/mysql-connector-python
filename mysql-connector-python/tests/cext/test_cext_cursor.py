...
class CMySQLCursorPreparedNamedTupleTests(CMySQLCursorPreparedTests):
    ...
    def test___init__(self):
        self.assertIsInstance(self.cur, CMySQLCursorPreparedNamedTuple)

    def test_close(self):
        cur = self.cnx.cursor(prepared=True, named_tuple=True)
        self.assertEqual(None, cur._stmt)
        cur.close()
    ...
    def test_fetchone(self):
        ...
        self.cur.execute("SELECT * FROM {0}".format(self.tbl))
        row = self.cur.fetchone()
        self.assertEqual(row[1:], self.named_tuple(*self.exp))
        row = self.cur.fetchone()
        self.assertIsNone(row)
    ...
    def test_fetchmany(self):
        ...
        rows = self.cur.fetchmany(1)
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0][1:], self.named_tuple(*self.exp))
...
class CMySQLCursorPreparedRawTests(CMySQLCursorPreparedTests):
    ...
    def test___init__(self):
        self.assertIsInstance(self.cur, CMySQLCursorPreparedRaw)

    def test_close(self):
        cur = self.cnx.cursor(prepared=True, raw=True)
        self.assertEqual(None, cur._stmt)
        cur.close()
